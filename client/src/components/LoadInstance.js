...
class LoadInstance extends React.Component {
  ...
  fileUpload(file) {
    ...
    let r = axios.post(url, formData, config);
    return r;
  }
    ...
    axios.get(`http://localhost:5000/library?processID=` + this.state.templateID).then(
      (response) => {
        ...
        if (result.data !== "KO") {
          ...
          this.setState({ public_var: result.data['public_var'] });
        }
      },
      (error) => {
        console.log(error);
                ...
                <div className='container'>
                  ...
                  <Row>
                    <Col>
                      ...
                      <Card>
                        <Card.Title>Public Variables</Card.Title>
                        <Card.Body>
                          <table>
                            {
                              this.state.public_var.map((value, j) => {
                                return (
                                  <>
                                    <tr>
                                      <td>Variable Name : {value.name}</td>
                                    </tr>
                                    <tr>
                                      {
